...
#Added waiting image
waiting_image = image.load("play_again.png")
...
#Added waiting & start time variables
waiting = False
...
while True:
    ...
    if waiting:
        ...
        #Added way to quit from waiting
        if new_event.type == KEYDOWN and new_event.key == K_q:
            quit()
            break
        #Added way to restart game from waiting
        if new_event.type == KEYDOWN and new_event.key == K_RETURN:
            ...
            pipe3_y = 400
            pipe3_flipped = True
            #Here we set waiting and start time again
            waiting = False
            start_time = time.get_ticks()
        continue
    ...
    if bird.colliderect(pipe) or bird.colliderect(pipe2) or bird.colliderect(pipe3):
        print("Game Over!")
        #Made the score be based on the start time
        score = (time.get_ticks() - start_time ) //1000
        print("You lasted:", score, "seconds!")
        #Set waiting at the end of each game
        waiting = True
